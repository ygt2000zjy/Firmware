...
extern "C" __EXPORT int xhy_att_controlsw_main(int argc, char *argv[]);
...
private:
  ...
  int 	_battery_status_sub;  //电池状态订阅
    ...
    math::Matrix<3, 3>	_board_rotation = {};	//飞控板朝向的旋转矩阵
		...
		float What_prev = 0;
		float What= 0;
		math::Vector<3> thetahat_prev={};
		math::Vector<3> thetahat={};
		math::Vector<3> sigmahat_prev={};
		math::Vector<3> sigmahat={};
    ...
    struct {
    	...
    	param_t 	control_allocate8;
			...
			//定义SW里面的一些参数
			param_t epsinonw;
			param_t epsinontheta;
			param_t epsinonsigma;
			param_t deltaw;
			param_t deltatheta;
			param_t deltasigma;
//下面这三个变量反应了这个公式，其实就是kg -inv(inv(Am2)*inv([Ixx 0 0;0 Iyy 0;0 0 Izz]))
	...
	//定义一个取三个数中绝对值最大的函数
	float xinf(float a, float b, float c);
	//基准控制器
...
XhyAttitudeControl::XhyAttitudeControl():
   ...
   //运行情况计数
	...
	_loop_update_rate_hz(initial_update_rate_hz) //这个有关回路速率的貌似最后也没用上
	{
		...
		_params.kg3 = 0.176f;
		//定义的与SW算法的参数有关变量
		_params.epsinonw = 0.00001f;
		_params.epsinontheta = 0.00001f;
		_params.epsinonsigma = 0.00001f;
		_params.deltaw = 2.0f;
		_params.deltatheta = 0.11f;
		_params.deltasigma = 0.11f;
		...
		_params_handles.lowpassfilter_param 	=	param_find("LOWPASSFILTER");
		...
		//与SW算法相关的参数
		_params_handles.epsinonw 	=	param_find("EPSIONW");
		_params_handles.epsinontheta 	=	param_find("EPSIONTHETA");
		_params_handles.epsinonsigma 	=	param_find("EPSIONSIGMA");
		_params_handles.deltaw 	=	param_find("DELTAW");
		_params_handles.deltatheta 	=	param_find("DELTATHETA");
		_params_handles.deltasigma 	=	param_find("DELTASIGMA");

		_params_handles.p11 	=	param_find("P11");
		_params_handles.p22 	=	param_find("P22");
		_params_handles.p33 	=	param_find("P33");
	...
	float
	XhyAttitudeControl::xinf(float a, float b, float c)
	{
		float aa;
		float bb;
		float cc;
	...
	XhyAttitudeControl::parameters_update()
	{
		...
		param_get(_params_handles.epsinonw, &_params.epsinonw);
		param_get(_params_handles.epsinontheta, &_params.epsinontheta);
		param_get(_params_handles.epsinonsigma, &_params.epsinonsigma);
		param_get(_params_handles.deltaw, &_params.deltaw);
		param_get(_params_handles.deltatheta, &_params.deltatheta);
		param_get(_params_handles.deltasigma, &_params.deltasigma);

		param_get(_params_handles.p11, &_params.p11);
		param_get(_params_handles.p22, &_params.p22);
		param_get(_params_handles.p33, &_params.p33);
...
XhyAttitudeControl::adaptive_control(float dt)
{
	...
	math::Vector<3> omega;   //飞行器真实角速率
	omega(0) = _v_att.rollspeed;
	omega(1) =  _v_att.pitchspeed;
	omega(2) = _v_att.yawspeed;

	math::Vector<3> omegadesire;
	omegadesire = _rates_sp;    //飞行器角速率期望值

	math::Matrix<3,3> 	invJ;    //转动惯量的逆
	...
	//这里使用欧拉法进行观测器的离散化
	math::Vector<3> omegahat;    //定义的角速率的观测变量

	math::Matrix<3,3> Am2;      //角速率的期望收敛性能
	Am2(0,0) =_params.Am2_outer;
	Am2(0,1) =0;
	Am2(0,2) =0;
	Am2(1,0) =0;
  ...
  math::Matrix<3,3> What_prev_matrix;
	What_prev_matrix(0,0) = What_prev;
	What_prev_matrix(0,1) = 0;
	What_prev_matrix(0,2) = 0;
	What_prev_matrix(1,0) = 0;
	What_prev_matrix(1,1) = What_prev;
	What_prev_matrix(1,2) = 0;
	What_prev_matrix(2,0) = 0;
	What_prev_matrix(2,1) = 0;
	What_prev_matrix(2,2) = What_prev;

	float omegahat_prev_inf = xinf(omegahat_prev(0), omegahat_prev(1), omegahat_prev(2));
	/*用来测试三个数绝对值找最大的函数，也就是变相的在求无穷范数
	float test = xinf(-6.0f,-4.0f,-10.0f);
	warnx("look at here %4.2f",(double)test);*/
	//状态观测器
	omegahat = (Am2 * h + _I)* omegahat_prev + invJ * h *( What_prev_matrix * Madaptive_prev + thetahat_prev * omegahat_prev_inf + sigmahat_prev);

	//自适应率
	math::Matrix<1,3> omegaerror_temp;
	math::Vector<3> omegaerror;
	omegaerror = omegahat - omega;
	omegaerror_temp(0,0) = omegaerror(0);
	omegaerror_temp(0,1) = omegaerror(1);
	omegaerror_temp(0,2) = omegaerror(2);

	math::Matrix<3,1> Madaptive_prev_temp;
	Madaptive_prev_temp(0,0) = Madaptive_prev(0);
	Madaptive_prev_temp(1,0) = Madaptive_prev(1);
	Madaptive_prev_temp(2,0) = Madaptive_prev(2);

	math::Matrix<1,1> tempW_temp;
	tempW_temp= omegaerror_temp * P * invJ * Madaptive_prev_temp;
		float tempW = tempW_temp(0,0);
			if ((double)fabs(tempW) > (double)_params.epsinonw)
					What = - _params.deltaw * tempW;
			else
					What = 0;

	 math::Vector<3> temptheta;
	 math::Matrix<1,3> t1;
	 t1 = omegaerror_temp * P *invJ;
	 temptheta(0) = t1(0,0)*xinf(omega(0),omega(1),omega(2));
	 temptheta(1) = t1(1,0)*xinf(omega(0),omega(1),omega(2));
	 temptheta(2) = t1(2,0)*xinf(omega(0),omega(1),omega(2));
	// temptheta = omegaerror_temp * P *invJ * xinf(omega(0),omega(1),omega(2));
		if ((double)fabs(temptheta(0)) > (double)_params.epsinontheta)
						thetahat(0) = - _params.deltatheta * temptheta(0);
				else
						thetahat(0) = 0;

		if ((double)fabs(temptheta(1)) > (double)_params.epsinontheta)
						thetahat(1) = - _params.deltatheta * temptheta(1);
				else
						thetahat(1) = 0;

		if ((double)fabs(temptheta(2)) > (double)_params.epsinontheta)
					  thetahat(2) = - _params.deltatheta * temptheta(2);
				else
						thetahat(2) = 0;

 		math::Vector<3> tempsigma;
		math::Matrix<1,3> t2;
		t2 = omegaerror_temp * P * invJ;
		 tempsigma(0) = t2(0,0);
		 tempsigma(1) = t2(1,0);
		 tempsigma(2) = t2(2,0);
  //  tempsigma = omegaerror_temp * P * B;
		if ((double)fabs(tempsigma(0)) > (double)_params.epsinonsigma)
						sigmahat(0) = - _params.deltasigma * tempsigma(0);
				else
						sigmahat(0) = 0;
		if ((double)fabs(tempsigma(1)) > (double)_params.epsinonsigma)
						sigmahat(1) = - _params.deltasigma * tempsigma(1);
				else
						sigmahat(1) = 0;

		if ((double)fabs(tempsigma(2)) > (double)_params.epsinonsigma)
						sigmahat(2) = - _params.deltasigma * tempsigma(2);
				else
						sigmahat(2) = 0;
//做完了上面这些事情，也就得到了三个在线估计参数的新值，旧值在哪用了一定要搞清
  ...
  kg(0,0) =_params.kg1;
	...
	//控制律
	Madaptive = (-TEMP2 * TEMP3 + _I) * Madaptive_prev + (-TEMP2)*(thetahat * xinf(omega(0),omega(1),omega(2)) + sigmahat - kg * omegadesire);
	What_prev= What;
	thetahat_prev = thetahat;
	sigmahat_prev = sigmahat;
...
{
	math::Vector<3> Mtotal;
	//Servo的前两个量是舵面，后两个量是电机
	math::Vector<4> Servo;
	Mtotal = Madaptive + Mbase;
  float det1;
	float det2;
	float deltaT1;
	float deltaT2;
	float det1_normalize;
	float det2_normalize;
	float deltaT1_normalize;
	float deltaT2_normalize;
	float T1;
	float T2;
  //控制分配矩阵的伪逆
	math::Matrix<4,3> pinvB;
	pinvB(0,0) = 0;
	pinvB(0,1) = -1.7042;
	pinvB(0,2) = -2.6983;
	pinvB(1,0) = 0;
	...
	deltaT1 = Servo(2);
	deltaT2 = Servo(2);
	det1_normalize = det1/(25.0f/57.3f);
	det2_normalize = det2/(25.0f/57.3f);
	deltaT1_normalize = deltaT1 / (float)9.0; //这里的电机拉力按最大0.9kg,也就是9N取的
  deltaT2_normalize = deltaT2 / (float)9.0;
	//做完了上面的归一化，这些值都变成了-1~1之间的值了
	T1 = deltaT1_normalize ;
	T2 = deltaT2_normalize ;
	warnx("look at here %f,%f",(double)T1,(double)T2);
	_actuators.control[0] = det1_normalize;
	_actuators.control[1] = det2_normalize;
...
int xhy_att_controlsw_main(int argc, char *argv[])
{
	if (argc < 2){
